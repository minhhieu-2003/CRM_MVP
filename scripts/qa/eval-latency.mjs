...
console.log("Smoke test passed: The `/api/chat` endpoint correctly returns the `{ reply, sources, context }` contract.\n");
...
for (const testCase of chatCases) {
  ...
  const promptText = testCase.type === "chat" ? testCase.prompt : `Sequence (${testCase.prompts.length} prompts)`;
  console.log(`| ${testCase.id} | ${testCase.type} | ${promptText} | ${latency} | ${passed ? "✅ PASS" : "❌ FAIL"} |`);
}
